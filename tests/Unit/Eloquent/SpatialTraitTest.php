...
use Tests\Unit\Helpers\TestModel;
...
class SpatialTraitTest extends BaseTestCase
{
    ...
    public function testInsertUpdatePointHasCorrectSql()
    {
        ...
        $this->assertContains('insert into `test_models` (`point`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        ...
        $this->assertContains('update `test_models` set `point` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testInsertUpdateLineStringHasCorrectSql()
    {
        ...
        $this->assertStringStartsWith('insert', $this->queries[0]);
        $this->assertContains('insert into `test_models` (`linestring`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        // TODO: assert bindings in query
        $this->assertTrue($this->model->exists);
        ...
        $this->assertStringStartsWith('update', $this->queries[1]);
        $this->assertContains('update `test_models` set `linestring` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testInsertUpdatePolygonHasCorrectSql()
    {
        ...
        $this->assertStringStartsWith('insert', $this->queries[0]);
        $this->assertContains('insert into `test_models` (`polygon`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        // TODO: assert bindings in query
        $this->assertTrue($this->model->exists);
        ...
        $this->model->save();
        $this->assertStringStartsWith('update', $this->queries[1]);
        $this->assertContains('update `test_models` set `polygon` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testInsertUpdateMultiPointHasCorrectSql()
    {
        ...
        $this->assertStringStartsWith('insert', $this->queries[0]);
        $this->assertContains('insert into `test_models` (`multipoint`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        // TODO: assert bindings in query
        $this->assertTrue($this->model->exists);
        ...
        $this->assertStringStartsWith('update', $this->queries[1]);
        $this->assertContains('update `test_models` set `multipoint` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testInsertUpdateMultiLineStringHasCorrectSql()
    {
        ...
        $this->model->multilinestring = new \Grimzy\LaravelMysqlSpatial\Types\MultiLineString([
            $linestring1, $linestring2,
        ]);
        $this->model->save();

        $this->assertStringStartsWith('insert', $this->queries[0]);
        $this->assertContains('insert into `test_models` (`multilinestring`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        // TODO: assert bindings in query
        $this->assertTrue($this->model->exists);
        ...
        $this->model->save();
        $this->assertStringStartsWith('update', $this->queries[1]);
        $this->assertContains('update `test_models` set `multilinestring` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testInsertUpdateMultiPolygonHasCorrectSql()
    {
        ...
        $this->assertStringStartsWith('insert', $this->queries[0]);
        $this->assertContains('insert into `test_models` (`multipolygon`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        // TODO: assert bindings in query
        $this->assertTrue($this->model->exists);
        ...
        $this->model->save();
        $this->assertStringStartsWith('update', $this->queries[1]);
        $this->assertContains('update `test_models` set `multipolygon` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testInsertUpdateGeometryCollectionHasCorrectSql()
    {
        ...
        $this->assertStringStartsWith('insert', $this->queries[0]);
        $this->assertContains('insert into `test_models` (`geometrycollection`) values (ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $this->queries[0]);
        // TODO: assert bindings in query
        $this->assertTrue($this->model->exists);
        ...
        $this->model->save();
        $this->assertStringStartsWith('update', $this->queries[1]);
        $this->assertContains('update `test_models` set `geometrycollection` = ST_GeomFromText(?, ?, \'axis-order=long-lat\') where `id` = ?',
            $this->queries[1]);
        // TODO: assert bindings in query
    }
    ...
    public function testScopeDistance()
    {
        ...
        $this->assertEquals('st_distance(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) <= ?',
            $q->wheres[0]['sql']);
    ...
    public function testScopeDistanceExcludingSelf()
    {
        ...
        $this->assertEquals('st_distance(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) <= ?',
            $q->wheres[0]['sql']);
        $this->assertEquals('st_distance(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) != 0',
            $q->wheres[1]['sql']);
        $this->assertEquals('POINT(2 1)', $bindings[0]);
        $this->assertEquals(10, $bindings[2]);
    ...
    public function testScopeDistanceSphere()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertEquals('st_distance_sphere(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) <= ?',
            $q->wheres[0]['sql']);
        $this->assertEquals('POINT(2 1)', $bindings[0]);
        $this->assertEquals(10, $bindings[2]);
    }
    ...
    public function testScopeDistanceSphereExcludingSelf()
    {
        ...
        $this->assertEquals('st_distance_sphere(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) <= ?',
            $q->wheres[0]['sql']);
        $this->assertEquals('st_distance_sphere(point, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) != 0',
            $q->wheres[1]['sql']);
        $this->assertEquals('POINT(2 1)', $bindings[0]);
        $this->assertEquals(10, $bindings[2]);
        $this->assertEquals('POINT(2 1)', $bindings[3]);
    ...
    public function testScopeDistanceValue()
    {
        ...
        $this->assertEquals('st_distance(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) as distance',
            $q->columns[1]->getValue());
        $this->assertEquals('POINT(2 1)', $bindings[0]);
    }
    ...
    public function testScopeDistanceSphereValue()
    {
        ...
        $this->assertEquals('*', $q->columns[0]);
        $this->assertInstanceOf(Expression::class, $q->columns[1]);
        $this->assertEquals('st_distance_sphere(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) as distance',
            $q->columns[1]->getValue());
        $this->assertEquals('POINT(2 1)', $bindings[0]);
    }
    ...
    public function testScopeComparison()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_within(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
    ...
    public function testScopeCrosses()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_crosses(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeContains()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_contains(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeDisjoint()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_disjoint(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeEquals()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_equals(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeIntersects()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_intersects(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeOverlaps()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_overlaps(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeDoesTouch()
    {
        ...
        $bindings = $q->getRawBindings()['where'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_touches(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\'))',
            $q->wheres[0]['sql']);
        $this->assertEquals('POLYGON((1 1,2 1),(2 1,2 2),(2 2,1 1))', $bindings[0]);
    }
    ...
    public function testScopeOrderByDistance()
    {
        ...
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_distance(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) asc',
            $q->orders[0]['sql']);
        $this->assertEquals('POINT(2 1)', $bindings[0]);
    }
    ...
    public function testScopeOrderByDistanceSphere()
    {
        ...
        $bindings = $q->getRawBindings()['order'];
        $this->assertNotEmpty($bindings);
        $this->assertContains('st_distance_sphere(`point`, ST_GeomFromText(?, ?, \'axis-order=long-lat\')) asc',
            $q->orders[0]['sql']);
        $this->assertEquals('POINT(2 1)', $bindings[0]);
    }
}
